<script context="module">
	...
	export function setGroup(new_number) {
		if (new_number > 0 && new_number <= 999999) group = parseInt(new_number);
	}
				...
				startPlayPromise
					...
					.catch((error) => {
						if (error.name === 'NotAllowedError') {
							alert('Autoplay ist blockiert. Du kannst die anderen Besucher*innen nicht hören.');
						} else {
...
<script>
	...
	import * as Janus from '../../node_modules/janus-gateway/html/janus';
		...
		await Janus.init({
			...
			callback: function () {
				if (!Janus.isWebrtcSupported()) {
					...
					error = encodeURIComponent('Dein Browser unterstützt kein WebRTC.');
				...
				janus = new Janus({
					...
					success: function () {
						...
						janus.attach({
							...
							error: function (cause) {
								Janus.error(' ::: Got an error while janus.attach() :::', cause);
								error = encodeURIComponent('Der Audiochat macht Probleme.');
								goto('/sorry?error=' + error);
							},
							consentDialog: function (state) {
								showConsentDialog = state;
								Janus.debug(' ::: showConsentDialog is ' + showConsentDialog + ' :::');
							},
							webrtcState: function (state, description) {
								if (state) {
									Janus.debug(' ::: webrtc state is true :::');
								} else {
									Janus.error(' ::: webrtc state is false :::', description);
								}
							},
							iceState: function (state) {
								Janus.debug(' ::: iceState is ' + state + ' :::');
							},
							mediaState: function (state) {
								Janus.debug(' ::: mediaState is ' + state + ' :::');
							},
									...
									if (msg['participants'] !== undefined && msg['participants'] !== null) {
										...
										if (list.length >= 20) {
											if (x >= 1234 && x <= 1253) {
												...
												goto('/connected?group=' + nextPublicGroup);
											} else {
												alert(
					...
					error: async function (cause) {
						// Error, can't go on...
						Janus.error(' ::: Got an error while new Janus() :::', cause);
						await goto('/sorry');
					},
					destroyed: async function () {
						// I should get rid of this
						Janus.debug(' ::: Got a destroyed notification :::');
						await goto('/');
					}
				});
			}
